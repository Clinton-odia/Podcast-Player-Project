document.addEventListener("DOMContentLoaded", () => {
  ...
  function toggleFavorite(podcast, iconEl) {
    if (isFavorited(podcast)) {
      favoritePodcasts = favoritePodcasts.filter(
        (p) => p.itunesId !== podcast.itunesId,
      );
      if (iconEl) {
  ...
  function createCard(podcast) {
    ...
    favBtnIcon.addEventListener("click", (e) => {
      ...
      favBtnIcon.title = isFavorited(podcast)
        ? "Remove Favorite"
        : "Add Favorite";
    });
  ...
  async function loadEpisodes(feedId, count) {
    ...
    try {
      const response = await fetch(
        `/api/episodes?feedId=${feedId}&max=${count}`,
      );
      const data = await response.json();
